...
class PostProductionQualityRC2:
    ...
    def analyze(self) -> dict[str, Any]:
        ...
        if total_duration > max_film:
            issues.append({
                ...
                "recommendation": "Сформировать Director Cut и сократить слабые фрагменты.",
            })
        ...
        if weak_opening:
            issues.append({
                ...
                "reason": (
                    ...
                    f"{len(opening_assets)} уникальных ассетов, {opening_video} видео."
                ),
                "recommendation": "Усилить начало движением, быстрыми сменами и выразительным звуком.",
            })
        ...
        for row in static_overruns:
            issues.append({
                ...
                "recommendation": f"Сократить до {max_static:g} сек. или добавить движение.",
        ...
        for asset, indexes in overused.items():
            issues.append({
                ...
                "recommendation": "Оставить лучшие появления, остальные заменить.",
                "asset_id": asset,
            })
        ...
        for item in rapid_reuse:
            issues.append({
                ...
                "recommendation": "Увеличить интервал или заменить повтор.",
                "asset_id": item["asset"],
            })
        ...
        for row in excluded:
            issues.append({
                ...
                "recommendation": "Удалить материал из таймлайна и заменить.",
                "asset_id": row["asset_id"],
                "timeline_index": row["index"],
            })
        ...
        if video_with_audio and not natural_audio:
            issues.append({
                ...
                "reason": f"Видео с исходным звуком: {len(video_with_audio)}; используется: 0.",
                "recommendation": "Добавить окна натурального звука и приглушение диктора.",
            })
    ...
    def _normalize(row: dict[str, Any], index: int) -> dict[str, Any]:
        ...
        return {
            ...
            "asset_id": asset_id,
            "asset_key": asset_key,
            "excluded": bool(row.get("excluded") or row.get("is_excluded") or status == "excluded"),
            "rejected": bool(row.get("rejected") or row.get("is_rejected") or status == "rejected"),
