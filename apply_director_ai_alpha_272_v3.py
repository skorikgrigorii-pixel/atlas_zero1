...
MODULE_TARGET = Path("src/az_enterprise/core/visual_dynamics_alpha272.py")
...
def create_backup(path: Path, stamp: str) -> Path | None:
    if not path.exists():
        return None
    backup_path = path.with_name(f"{path.name}.alpha272v3.{stamp}.bak")
...
def patch_module(source: str) -> str:
    ...
    if "enrich_visual_dynamics" in source:
        raise RuntimeError("Visual Dynamics integration already exists.")
    ...
    wrapper = (
        "\n"
        f"{indent}def analyze(self):\n"
        f"{indent}    from .visual_dynamics_alpha272 import "
        "enrich_visual_dynamics\n"
        f"{indent}    base_result = self._analyze_alpha271()\n"
        f"{indent}    return enrich_visual_dynamics(\n"
        f"{indent}        config=self.config,\n"
        f"{indent}        base_result=base_result,\n"
        f"{indent}        normalize=self._normalize,\n"
        f"{indent}    )\n"
    )

    lines.insert(analyze_node.end_lineno, wrapper)
...
def main() -> int:
    ...
    print("Director AI Alpha 2.7.2 v3 applied successfully.")
    for path in tracked:
        backup = backups[path]
        if backup:
