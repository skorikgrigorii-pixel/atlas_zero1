...
$RuntimePath = Join-Path $Root "src\az_enterprise\core\finalization_runtime_rc2.py"
$TestPath = Join-Path $Root "tests\test_finalization_runtime_rc2.py"
$Timestamp = Get-Date -Format "yyyyMMdd_HHmmss"

if (-not (Test-Path (Join-Path $Root "src\az_enterprise\core"))) {
    throw "Run this installer from the atlas_zero1 repository root."
}

foreach ($Path in @($RuntimePath, $TestPath)) {
    if (Test-Path $Path) {
...
New-Item (Split-Path $RuntimePath) -ItemType Directory -Force | Out-Null
New-Item (Split-Path $TestPath) -ItemType Directory -Force | Out-Null
...
python -m py_compile $RuntimePath $TestPath

Write-Host ""
Write-Host "=== FINALIZATION TESTS ==="
python -m pytest $TestPath -q

Write-Host ""
Write-Host "=== INSTALLED ==="
Get-Item $RuntimePath, $TestPath |
    Select-Object FullName, Length, LastWriteTime

Write-Host ""
Write-Host "Commands:"
Write-Host "  python -m az_enterprise.core.finalization_runtime_rc2 approve-preview hogueras"
Write-Host "  python -m az_enterprise.core.finalization_runtime_rc2 promote-preview hogueras --as-final"
