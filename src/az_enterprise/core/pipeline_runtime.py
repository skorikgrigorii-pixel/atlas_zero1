...
class PipelineRunManager:
    """Enterprise RC1 Alpha 2.4 runtime.
    ...
    def _export_pipeline_graph(self, run_id: int) -> None:
        ...
        (self.export_dir / "pipeline_graph.json").write_text(json.dumps({"run_id": run_id, "version": "RC1 Alpha 2.4", "nodes": rows}, ensure_ascii=False, indent=2), encoding="utf-8")
...
<h1>ATLAS ZERO — Pipeline Graph / Run #{run_id}</h1><div class='grid'>{cards}</div>"""
    ...
    def _export_run_report(self, run_id: int, result: dict[str, Any]) -> None:
        ...
        report = {"version": "RC1 Alpha 2.4", "run_id": run_id, "project_id": self.project_id, "status": "done", "tasks": tasks, "events": events, "logs": logs, "result": result}
...
<h1>ATLAS ZERO — Pipeline Run #{run_id}</h1><div class='card'>Статус: <b>done</b><br>Версия: RC1 Alpha 2.4<br>Файлов после запуска: {result.get('counts_after',{})}</div><h2>Очередь задач</h2><table><tr><th>#</th><th>Task ID</th><th>Этап</th><th>Статус</th><th>Попытки</th><th>Прогресс</th></tr>{html_rows}</table><h2>Журнал</h2><div class='card'>{html_logs}</div>""",
            encoding="utf-8",
        )
